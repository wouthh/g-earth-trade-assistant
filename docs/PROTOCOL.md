...
Blocked inbound room/rights messages still invalidate the send fence and discard room context: the server transition already happened even when another extension hides it from the client. Blocked outgoing navigation has not reached the server and is ignored. Malformed relevant observations require fresh room context. Room additions stop being recorded at 100,000 unique IDs; the overflow halt is emitted once and re-arming requires a new context.

## Folder launcher contract

The pinned public [`StoreExtensionTools.executeExtension`](https://github.com/G-Realm/G-Earth/blob/b993d5ba0b23ab5644633abb8074229d1cb53b71/G-Earth/src/main/java/gearth/app/services/internal_extensions/extensionstore/tools/StoreExtensionTools.java) reads `command.txt` from the installed folder, substitutes the host arguments, and sets the process working directory to its `extension/` child. The distribution places the JAR in that child and tests a real process launch from an extracted ZIP. An absolute Java executable preserves the selected extension runtime when the host uses an older Java version.

Cancellation and the local transport write share one critical section; packet composition and journal I/O happen outside it. Cancellation may await an already executing localhost write, never a hotel acknowledgement. A stale permit produces the factual `CANCELLED` result without invoking transport. Placement reservations are then released without creating a removal tombstone; redemption intent is cleared while the placed object remains observed. `UNKNOWN` transport results retain the conservative recovery stop.
